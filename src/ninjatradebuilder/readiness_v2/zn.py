...
from ..schemas.contracts import ZNContractSpecificExtension
...
def build_zn_premarket_briefing(
    packet: HistoricalPacket | Mapping[str, Any],
    *,
    created_at: datetime | None = None,
) -> PremarketBriefingV1:
    validated = _coerce_zn_packet(packet)
...
def build_zn_initial_query_triggers(
    packet: HistoricalPacket | Mapping[str, Any],
    briefing: PremarketBriefingV1,
) -> list[QueryTriggerV1]:
    validated = _coerce_zn_packet(packet)
    if briefing.contract != "ZN":
        raise ValueError("ZN readiness v2 only accepts ZN briefings.")
...
def build_zn_initial_readiness_watch(
    packet: HistoricalPacket | Mapping[str, Any],
    briefing: PremarketBriefingV1,
...
def update_zn_readiness_watch(
    prior_watch: ReadinessWatchV1 | Mapping[str, Any],
    packet: HistoricalPacket | Mapping[str, Any],
    *,
    evaluation_timestamp: datetime,
) -> ReadinessWatchV1:
    prior = _coerce_zn_watch(prior_watch)
    validated = _coerce_zn_packet(packet)
...
def replay_zn_readiness_sequence(
    packet: HistoricalPacket | Mapping[str, Any],
    ...
    *,
    ...
    validated = _coerce_zn_packet(packet)
    briefing = build_zn_premarket_briefing(validated, created_at=bootstrap_evaluation_time)
    query_triggers = build_zn_initial_query_triggers(validated, briefing)
    watch = build_zn_initial_readiness_watch(
        validated,
        briefing,
        query_triggers,
    ...
    for step_index, update in enumerate(updates, start=1):
        ...
        watch = update_zn_readiness_watch(
            watch,
            update_packet,
...
def _coerce_replay_update(
    ...
    return shared_coerce_replay_update(update, packet_coercer=_coerce_zn_packet)
...
def _build_watch_context_snapshot(
    ...
    if not isinstance(extension, ZNContractSpecificExtension):
        raise ValueError("ZN readiness v2 requires a ZN contract extension.")
    ...
    return ZNReadinessWatchContextSnapshot(
        evaluation_timestamp=evaluation_time,
        ...
        auction_proximity_state=_auction_proximity_state(packet, evaluation_time, next_event),
        next_event_name=next_event.name if next_event else None,
        next_event_time=_as_utc(next_event.time) if next_event else None,
        minutes_until_next_event=_minutes_until(next_event.time, evaluation_time) if next_event else None,
...
def _build_feature_snapshot(packet: HistoricalPacket) -> ZNPremarketBriefingFeatureSnapshot:
    extension = packet.contract_specific_extension
    if not isinstance(extension, ZNContractSpecificExtension):
        raise ValueError("ZN readiness v2 requires a ZN contract extension.")

    return ZNPremarketBriefingFeatureSnapshot(
        current_price=packet.market_packet.current_price,
...
def _build_thesis_claims(
    ...
    flow_evidence_fields = [
        "market_packet.cumulative_delta",
        "market_packet.current_volume_vs_average",
    ]
    if extension.absorption_summary:
        flow_evidence_fields.append("contract_specific_extension.absorption_summary")

    event_statement = (
        f"The next scheduled event is {next_event.name} at {next_event.time.isoformat()}, "
        "which can hard-stop the watch as the lockout window approaches."
    ...
    claims = [
        NarrativeClaim(
            ...
            provenance=[
                _field_provenance(
                    ...
                    "Current ZN price anchors the value-context portion of the thesis.",
                ...
                _field_provenance(
                    "current_session_poc",
                    "market_packet.current_session_poc",
                    _format_price(packet.market_packet.current_session_poc),
                    "Session POC anchors the accepted value reference in the thesis.",
                    observed_at=_as_utc(packet.market_packet.timestamp),
                ),
                _field_provenance(
                    "prior_day_high",
                    "market_packet.prior_day_high",
                    _format_price(packet.market_packet.prior_day_high),
                    "Prior-day high is the next upside reference in the thesis.",
                    observed_at=_as_utc(packet.market_packet.timestamp),
                ),
                _derived_provenance(
                    "directional_lean",
        ...
        NarrativeClaim(
            ...
            category="flow",
            statement=(
                f"Cumulative delta is {packet.market_packet.cumulative_delta:.0f} with "
                f"volume running {packet.market_packet.current_volume_vs_average:.2f}x average."
            ),
            evidence_fields=flow_evidence_fields,
            provenance=[
                _field_provenance(
                    "cumulative_delta",
                    "market_packet.cumulative_delta",
            ...
            + (
                [
                    _field_provenance(
                        ...
                        extension.absorption_summary,
                        "Absorption context supports the flow alignment claim.",
                        observed_at=evaluation_time,
                    )
                ]
                if extension.absorption_summary
        ...
        NarrativeClaim(
            ...
            category="risk",
            statement=(
                f"Macro context is '{extension.macro_release_context}' with auction schedule "
                f"'{extension.treasury_auction_schedule}'."
            ),
            evidence_fields=[
                "contract_specific_extension.macro_release_context",
                "contract_specific_extension.treasury_auction_schedule",
                "contract_specific_extension.cash_10y_yield",
            ],
            provenance=[
                _field_provenance(
                    "macro_release_context",
                    "contract_specific_extension.macro_release_context",
                    extension.macro_release_context,
                    "Macro release posture is carried directly into the thesis.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "treasury_auction_schedule",
                    "contract_specific_extension.treasury_auction_schedule",
                    extension.treasury_auction_schedule,
                    "Treasury auction schedule is carried directly into the thesis.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "cash_10y_yield",
                    "contract_specific_extension.cash_10y_yield",
                    f"{extension.cash_10y_yield:.2f}",
                    "Cash 10Y yield anchors the fixed-income macro posture reference.",
                    observed_at=evaluation_time,
                ),
            ],
        ),
    ...
    if price_trigger is not None:
        summaries.append(
            BriefingTriggerSummary(
                ...
                summary=(
                    f"Escalate only if ZN tags {_format_price(price_trigger[1])}, "
...
def _build_lockout_conditions(
    ...
    if _session_access_state(packet, evaluation_time) == "OUTSIDE_ALLOWED_HOURS":
        conditions.append(
            ReadinessLockoutCondition(
                ...
                description="The watch is outside the ZN allowed-hours session window.",
                provenance=[
                    _field_provenance(
    ...
    conditions.append(
        ReadinessLockoutCondition(
            ...
            kind="SESSION_LOCKOUT",
            description="The watch hard-stops at the ZN allowed-hours close.",
            provenance=[
                _field_provenance(
...
def _build_price_touch_trigger(
    ...
    operator_claims = [
        NarrativeClaim(
            ...
            category="trigger",
            statement=(
                f"Escalate for fresh Stage A+B reanalysis if ZN tags {_format_price(level_price)} "
...
def _coerce_zn_watch(watch: ReadinessWatchV1 | Mapping[str, Any]) -> ReadinessWatchV1:
    validated = watch if isinstance(watch, ReadinessWatchV1) else ReadinessWatchV1.model_validate(watch)
    if validated.contract != "ZN":
        raise ValueError("Readiness v2 first slice only supports ZN watches.")
    return validated
...
def _build_watch_state_claim(
    ...
    *,
    ...
    if state == "READY_FOR_REANALYSIS":
        statement = (
            f"ZN watch is ready_for_reanalysis after {active_trigger_family} observed at "
        ...
        statement = (
            f"ZN watch is context_contaminated because packet freshness is {context_snapshot.freshness_state.lower()} "
            f"at age {context_snapshot.packet_age_seconds} seconds."
        )
    elif state == "LOCKED_OUT":
        statement = (
            f"ZN watch is locked_out because deterministic lockout reasons are "
            f"{', '.join(context_snapshot.lockout_reasons)}."
        )
    elif state == "THESIS_INVALIDATED":
        statement = (
            f"ZN watch is thesis_invalidated because current price {_format_price(packet.market_packet.current_price)} "
            f"breached the active invalidation level."
        )
    elif state == "TERMINAL_EXPIRED":
        statement = (
            f"ZN watch is terminal_expired because evaluation time {_render_datetime(evaluation_time)} "
            f"reached the watch expiry."
        )
    elif active_trigger_family == "price_level_touch":
        statement = (
            f"ZN watch is {state.lower()} with price-touch monitoring at {_format_price(price_level or 0.0)} "
        ...
        statement = (
            f"ZN watch is {state.lower()} with a timed recheck at {_render_datetime(recheck_at_time or evaluation_time)} "
...
def _detect_watch_changes(
    ...
    *,
    ...
    _append_change(
        changes,
        kind="auction_proximity_change",
        from_value=prior_watch.context_snapshot.auction_proximity_state,
        to_value=current_context.auction_proximity_state,
        detail=(
            f"Treasury auction proximity moved from {prior_watch.context_snapshot.auction_proximity_state} "
            f"to {current_context.auction_proximity_state}."
        ),
    )
    _append_change(
        changes,
        ...
        from_value=(
            ...
            f"{prior_watch.context_snapshot.treasury_auction_schedule}"
        ),
        to_value=(
            f"{current_context.macro_release_context}|"
            f"{current_context.treasury_auction_schedule}"
        ),
        detail="Macro release posture changed on the validated packet.",
    )
    _append_change(
        changes,
...
def _structure_claim_text(
    ...
    if feature_snapshot.directional_lean == "BEARISH":
        return (
            f"ZN is trading below VWAP {_format_price(market.vwap)} with negative directional posture "
            f"toward the lower edge of value at {_format_price(market.current_session_val)}."
        )
    if feature_snapshot.directional_lean == "TWO_WAY":
        return (
            f"ZN is rotating inside developing value between {_format_price(market.current_session_val)} "
            f"and {_format_price(market.current_session_vah)} with no clean directional edge yet."
        )
    return (
        f"ZN is holding above VWAP {_format_price(market.vwap)} and session POC "
...
def _coerce_zn_packet(packet: HistoricalPacket | Mapping[str, Any]) -> HistoricalPacket:
    validated = packet if isinstance(packet, HistoricalPacket) else validate_historical_packet(packet)
    if validated.market_packet.contract != "ZN":
        raise ValueError("Readiness v2 first slice only supports ZN packets.")
    if not isinstance(validated.contract_specific_extension, ZNContractSpecificExtension):
        raise ValueError("Readiness v2 first slice requires a ZN contract extension.")
    return validated
...
def _artifact_id(kind: str, evaluation_time: datetime) -> str:
    return f"zn-{kind}-{evaluation_time.strftime('%Y%m%dT%H%M%SZ').lower()}"
...
def _validate_watch_triggers(
    ...
    if not triggers:
        raise ValueError("ZN readiness watch requires at least one query trigger.")

    seen_families: set[str] = set()
    for trigger in triggers:
        if trigger.contract != "ZN":
            raise ValueError("Readiness v2 first slice only supports ZN query triggers.")
...
def _governance_lock_reasons(packet: HistoricalPacket) -> list[str]:
    reasons: list[str] = []
    trades_today = packet.challenge_state.trades_today_by_contract.model_dump(by_alias=True)["ZN"]
    ...
    if any(position.contract == "ZN" for position in packet.challenge_state.current_open_positions):
        reasons.append("existing_zn_position_open")
    return reasons
