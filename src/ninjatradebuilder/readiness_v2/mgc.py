...
from ..schemas.contracts import MGCContractSpecificExtension
...
def build_mgc_premarket_briefing(
    packet: HistoricalPacket | Mapping[str, Any],
    *,
    created_at: datetime | None = None,
) -> PremarketBriefingV1:
    validated = _coerce_mgc_packet(packet)
...
def build_mgc_initial_query_triggers(
    packet: HistoricalPacket | Mapping[str, Any],
    briefing: PremarketBriefingV1,
) -> list[QueryTriggerV1]:
    validated = _coerce_mgc_packet(packet)
    if briefing.contract != "MGC":
        raise ValueError("MGC readiness v2 only accepts MGC briefings.")
...
def build_mgc_initial_readiness_watch(
    packet: HistoricalPacket | Mapping[str, Any],
    briefing: PremarketBriefingV1,
...
def update_mgc_readiness_watch(
    prior_watch: ReadinessWatchV1 | Mapping[str, Any],
    packet: HistoricalPacket | Mapping[str, Any],
    *,
    evaluation_timestamp: datetime,
) -> ReadinessWatchV1:
    prior = _coerce_mgc_watch(prior_watch)
    validated = _coerce_mgc_packet(packet)
...
def replay_mgc_readiness_sequence(
    packet: HistoricalPacket | Mapping[str, Any],
    ...
    *,
    ...
    validated = _coerce_mgc_packet(packet)
    briefing = build_mgc_premarket_briefing(validated, created_at=bootstrap_evaluation_time)
    query_triggers = build_mgc_initial_query_triggers(validated, briefing)
    watch = build_mgc_initial_readiness_watch(
        validated,
        briefing,
        query_triggers,
    ...
    for step_index, update in enumerate(updates, start=1):
        ...
        watch = update_mgc_readiness_watch(
            watch,
            update_packet,
...
def _build_feature_snapshot(packet: HistoricalPacket) -> MGCPremarketBriefingFeatureSnapshot:
    extension = packet.contract_specific_extension
    if not isinstance(extension, MGCContractSpecificExtension):
        raise ValueError("MGC readiness v2 requires an MGC contract extension.")

    return MGCPremarketBriefingFeatureSnapshot(
        ...
        macro_fear_catalyst_summary=_macro_fear_catalyst_summary(extension),
        swing_penetration_volume_summary=_swing_penetration_volume_summary(extension),
...
def _build_watch_context_snapshot(
    ...
    return MGCReadinessWatchContextSnapshot(
        evaluation_timestamp=evaluation_time,
...
def _build_thesis_claims(
    ...
    feature_snapshot: MGCPremarketBriefingFeatureSnapshot,
    evaluation_time: datetime,
) -> list[NarrativeClaim]:
    extension = packet.contract_specific_extension
    if not isinstance(extension, MGCContractSpecificExtension):
    ...
    event_statement = (
        f"The next scheduled event is {next_event.name} at {next_event.time.isoformat()}, which can hard-stop the MGC watch as the lockout window approaches."
    ...
    return [
        NarrativeClaim(
            ...
            provenance=[
                _field_provenance(
                    ...
                    "Current MGC price anchors the value-context portion of the thesis.",
                ...
                _field_provenance(
                    ...
                    "Current session VAH anchors the MGC upside value boundary.",
                    observed_at=_as_utc(packet.market_packet.timestamp),
                ),
                _field_provenance(
                    "current_session_val",
                    ...
                    "Current session VAL anchors the MGC downside value boundary.",
                    observed_at=_as_utc(packet.market_packet.timestamp),
                ),
                _derived_provenance(
                    "directional_lean",
        ...
        NarrativeClaim(
            ...
            category="flow",
            statement=(
                f"{_swing_penetration_volume_summary(extension)} Cumulative delta is "
            ...
            provenance=[
                _field_provenance(
                    ...
                    "contract_specific_extension.swing_penetration_volume_summary",
                    _swing_penetration_volume_summary(extension),
                    "Swing-penetration volume summary is carried directly into the MGC flow claim.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "cumulative_delta",
        ...
        NarrativeClaim(
            ...
            category="risk",
            statement=(
                f"DXY is '{feature_snapshot.dxy_context}', yields are '{feature_snapshot.yield_context}', "
                f"macro fear catalyst posture is '{feature_snapshot.macro_fear_catalyst_summary}', and "
                f"cross-market context is '{feature_snapshot.cross_market_context}'."
            ),
            evidence_fields=[
                "contract_specific_extension.dxy_context",
                "contract_specific_extension.yield_context",
            ...
            provenance=[
                _field_provenance(
                    "dxy_context",
                    "contract_specific_extension.dxy_context",
                    feature_snapshot.dxy_context,
                    "DXY context is taken directly from the MGC contract extension.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "yield_context",
                    "contract_specific_extension.yield_context",
                    feature_snapshot.yield_context,
                    "Yield context is taken directly from the MGC contract extension.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "macro_fear_catalyst_summary",
                    "contract_specific_extension.macro_fear_catalyst_summary",
                    feature_snapshot.macro_fear_catalyst_summary,
                    "Macro fear catalyst summary is taken directly from the MGC contract extension.",
                    observed_at=evaluation_time,
                ),
                _derived_provenance(
                    ...
                    "Cross-market context is deterministically rendered from the packet cross-market snapshot.",
                    observed_at=evaluation_time,
                ),
            ],
        ),
        NarrativeClaim(
            ...
            evidence_fields=[
                "session_snapshot.next_event_name",
                "session_snapshot.next_event_time",
            ],
            provenance=[
                _field_provenance(
                    "next_event_name",
                    ...
                    "The next event name anchors the event-risk statement.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "next_event_time",
                    "session_snapshot.next_event_time",
                    _render_datetime(_as_utc(next_event.time)) if next_event else "none",
                    "The next event time anchors the event-risk timing statement.",
                    observed_at=evaluation_time,
                ),
            ],
        ),
    ...
    if price_trigger is not None:
        summaries.append(
            BriefingTriggerSummary(
                ...
                summary=(
                    f"Escalate only if MGC tags {_format_price(price_trigger[1])}, "
...
def _build_price_touch_trigger(
    ...
    operator_claims = [
        NarrativeClaim(
            ...
            category="trigger",
            statement=(
                f"Escalate for fresh Stage A+B reanalysis if MGC tags {_format_price(level_price)} "
...
def _build_lockout_conditions(
    ...
    if _session_access_state(packet, evaluation_time) == "OUTSIDE_ALLOWED_HOURS":
        conditions.append(
            ReadinessLockoutCondition(
                ...
                description="The watch is outside the MGC allowed-hours session window.",
                provenance=[
                    _field_provenance(
    ...
    conditions.append(
        ReadinessLockoutCondition(
            ...
            kind="SESSION_LOCKOUT",
            description="The watch hard-stops at the MGC allowed-hours close.",
            provenance=[
                _field_provenance(
...
def _build_watch_state_claim(
    ...
    *,
    ...
    if state == "READY_FOR_REANALYSIS":
        statement = (
            f"MGC watch is ready_for_reanalysis after {active_trigger_family} observed at "
        ...
        statement = (
            f"MGC watch is context_contaminated because packet freshness is {context_snapshot.freshness_state.lower()} "
            f"at age {context_snapshot.packet_age_seconds} seconds."
        )
    elif state == "LOCKED_OUT":
        statement = (
            f"MGC watch is locked_out because deterministic lockout reasons are "
            f"{', '.join(context_snapshot.lockout_reasons)}."
        )
    elif state == "THESIS_INVALIDATED":
        statement = (
            f"MGC watch is thesis_invalidated because current price {_format_price(packet.market_packet.current_price)} "
            f"breached the active invalidation level."
        )
    elif state == "TERMINAL_EXPIRED":
        statement = (
            f"MGC watch is terminal_expired because evaluation time {_render_datetime(evaluation_time)} "
            f"reached the watch expiry."
        )
    elif active_trigger_family == "price_level_touch":
        statement = (
            f"MGC watch is {state.lower()} with price-touch monitoring at {_format_price(price_level or 0.0)} "
        ...
        statement = (
            f"MGC watch is {state.lower()} with a timed recheck at {_render_datetime(recheck_at_time or evaluation_time)} "
    ...
    *,
    ...
    _append_change(
        ...
        from_value=(
            ...
            f"{prior_watch.context_snapshot.yield_context}|"
            f"{prior_watch.context_snapshot.macro_fear_catalyst_summary}|"
            f"{prior_watch.context_snapshot.swing_penetration_volume_summary}|"
            f"{prior_watch.context_snapshot.cross_market_context}"
        ),
        to_value=(
            f"{current_context.dxy_context}|"
            f"{current_context.yield_context}|"
            f"{current_context.macro_fear_catalyst_summary}|"
            f"{current_context.swing_penetration_volume_summary}|"
            f"{current_context.cross_market_context}"
        ),
        detail="DXY, yields, macro fear posture, swing-penetration volume, or cross-market context changed on the validated packet.",
    )
    _append_change(
        changes,
...
def _structure_claim_text(
    ...
    if feature_snapshot.directional_lean == "BEARISH":
        return (
            f"MGC is trading below VWAP {_format_price(market.vwap)} with defensive posture "
            f"toward developing VAL at {_format_price(market.current_session_val)}."
        )
    if feature_snapshot.directional_lean == "TWO_WAY":
        return (
            f"MGC is rotating inside developing value between {_format_price(market.current_session_val)} "
            f"and {_format_price(market.current_session_vah)} with no clean directional edge yet."
        )
    return (
        f"MGC is holding above VWAP {_format_price(market.vwap)} and session POC "
...
def _cross_market_context(packet: HistoricalPacket) -> str:
    ...
    fragments: list[str] = []
    for key in ("equity_tone", "silver_direction", "dxy_direction", "bond_yield_direction"):
...
def _macro_fear_catalyst_summary(extension: MGCContractSpecificExtension) -> str:
    return extension.macro_fear_catalyst_summary


def _swing_penetration_volume_summary(extension: MGCContractSpecificExtension) -> str:
    return extension.swing_penetration_volume_summary or "Swing-penetration volume posture is unavailable."


def _coerce_mgc_packet(packet: HistoricalPacket | Mapping[str, Any]) -> HistoricalPacket:
    validated = packet if isinstance(packet, HistoricalPacket) else validate_historical_packet(packet)
    if validated.market_packet.contract != "MGC":
        raise ValueError("Readiness v2 MGC slice only supports MGC packets.")
    if not isinstance(validated.contract_specific_extension, MGCContractSpecificExtension):
        raise ValueError("Readiness v2 MGC slice requires an MGC contract extension.")
    return validated


def _coerce_mgc_watch(watch: ReadinessWatchV1 | Mapping[str, Any]) -> ReadinessWatchV1:
    validated = watch if isinstance(watch, ReadinessWatchV1) else ReadinessWatchV1.model_validate(watch)
    if validated.contract != "MGC":
        raise ValueError("Readiness v2 MGC slice only supports MGC watches.")
    return validated


def _artifact_id(kind: str, evaluation_time: datetime) -> str:
    return f"mgc-{kind}-{evaluation_time.strftime('%Y%m%dT%H%M%SZ').lower()}"
...
def _validate_watch_triggers(
    ...
    if not triggers:
        raise ValueError("MGC readiness watch requires at least one query trigger.")

    seen_families: set[str] = set()
    for trigger in triggers:
        if trigger.contract != "MGC":
            raise ValueError("Readiness v2 MGC slice only supports MGC query triggers.")
...
def _governance_lock_reasons(packet: HistoricalPacket) -> list[str]:
    reasons: list[str] = []
    trades_today = packet.challenge_state.trades_today_by_contract.model_dump(by_alias=True)["MGC"]
    ...
    if any(position.contract == "MGC" for position in packet.challenge_state.current_open_positions):
        reasons.append("existing_mgc_position_open")
    return reasons
...
def _coerce_replay_update(
    ...
    return shared_coerce_replay_update(update, packet_coercer=_coerce_mgc_packet)
