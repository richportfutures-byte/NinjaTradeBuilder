...
from ..schemas.contracts import ESContractSpecificExtension
...
def build_es_premarket_briefing(
    packet: HistoricalPacket | Mapping[str, Any],
    *,
    created_at: datetime | None = None,
) -> PremarketBriefingV1:
    validated = _coerce_es_packet(packet)
...
def build_es_initial_query_triggers(
    packet: HistoricalPacket | Mapping[str, Any],
    briefing: PremarketBriefingV1,
) -> list[QueryTriggerV1]:
    validated = _coerce_es_packet(packet)
    if briefing.contract != "ES":
        raise ValueError("ES readiness v2 only accepts ES briefings.")
...
def build_es_initial_readiness_watch(
    packet: HistoricalPacket | Mapping[str, Any],
    briefing: PremarketBriefingV1,
...
def update_es_readiness_watch(
    prior_watch: ReadinessWatchV1 | Mapping[str, Any],
    packet: HistoricalPacket | Mapping[str, Any],
    *,
    evaluation_timestamp: datetime,
) -> ReadinessWatchV1:
    prior = _coerce_es_watch(prior_watch)
    validated = _coerce_es_packet(packet)
...
def replay_es_readiness_sequence(
    packet: HistoricalPacket | Mapping[str, Any],
    ...
    *,
    ...
    validated = _coerce_es_packet(packet)
    briefing = build_es_premarket_briefing(validated, created_at=bootstrap_evaluation_time)
    query_triggers = build_es_initial_query_triggers(validated, briefing)
    watch = build_es_initial_readiness_watch(
        validated,
        briefing,
        query_triggers,
    ...
    for step_index, update in enumerate(updates, start=1):
        ...
        watch = update_es_readiness_watch(
            watch,
            update_packet,
...
def _coerce_replay_update(
    ...
    return shared_coerce_replay_update(update, packet_coercer=_coerce_es_packet)
...
def _build_watch_context_snapshot(
    ...
    if not isinstance(extension, ESContractSpecificExtension):
        raise ValueError("ES readiness v2 requires an ES contract extension.")
    ...
    return ESReadinessWatchContextSnapshot(
        evaluation_timestamp=evaluation_time,
        ...
        breadth=extension.breadth,
        index_cash_tone=extension.index_cash_tone,
...
def _build_feature_snapshot(
    ...
    return ESPremarketBriefingFeatureSnapshot(
        current_price=packet.market_packet.current_price,
        ...
        index_cash_tone=extension.index_cash_tone,
        macro_release_context=_macro_release_context(packet, evaluation_time),
        market_internals_context=_market_internals_context(extension),
...
def _build_thesis_claims(
    ...
    feature_snapshot: ESPremarketBriefingFeatureSnapshot,
    evaluation_time: datetime,
) -> list[NarrativeClaim]:
    extension = packet.contract_specific_extension
    if not isinstance(extension, ESContractSpecificExtension):
    ...
    event_statement = (
        f"The next scheduled event is {next_event.name} at {next_event.time.isoformat()}, which can hard-stop the ES watch as the lockout window approaches."
    ...
    claims = [
        NarrativeClaim(
            ...
            provenance=[
                _field_provenance(
                    ...
                    "Current ES price anchors the value-context portion of the thesis.",
                ...
                _field_provenance(
                    ...
                    "Current session VAH anchors the ES upside value boundary.",
                    observed_at=_as_utc(packet.market_packet.timestamp),
                ),
                _field_provenance(
                    "current_session_val",
                    ...
                    "Current session VAL anchors the ES downside value boundary.",
                    observed_at=_as_utc(packet.market_packet.timestamp),
                ),
                _derived_provenance(
                    "directional_lean",
        ...
        NarrativeClaim(
            ...
            category="flow",
            statement=(
                f"Breadth is {extension.breadth}, index cash tone is {extension.index_cash_tone}, cumulative delta is {packet.market_packet.cumulative_delta:.0f}, and volume is {packet.market_packet.current_volume_vs_average:.2f}x average."
            ),
            evidence_fields=[
            ...
            provenance=[
                _field_provenance(
                    ...
                    "contract_specific_extension.breadth",
                    extension.breadth,
                    "Breadth is carried directly into the ES flow alignment claim.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "index_cash_tone",
                    "contract_specific_extension.index_cash_tone",
                    extension.index_cash_tone,
                    "Index cash tone is carried directly into the ES flow alignment claim.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "cumulative_delta",
        ...
        NarrativeClaim(
            ...
            category="risk",
            statement=(
                f"Event context is '{feature_snapshot.macro_release_context}' while breadth is '{extension.breadth}' and cash tone is '{extension.index_cash_tone}'."
            ),
            evidence_fields=[
            ...
            provenance=[
                _derived_provenance(
                    ...
                    "Macro release context is deterministically derived from released and upcoming ES event calendar entries.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "breadth",
                    "contract_specific_extension.breadth",
                    extension.breadth,
                    "Breadth is a direct ES contract extension input.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "index_cash_tone",
                    "contract_specific_extension.index_cash_tone",
                    extension.index_cash_tone,
                    "Index cash tone is a direct ES contract extension input.",
                    observed_at=evaluation_time,
                ),
            ],
        ),
    ...
    if price_trigger is not None:
        summaries.append(
            BriefingTriggerSummary(
                ...
                summary=(
                    f"Escalate only if ES tags {_format_price(price_trigger[1])}, "
...
def _build_lockout_conditions(
    ...
    if _session_access_state(packet, evaluation_time) == "OUTSIDE_ALLOWED_HOURS":
        conditions.append(
            ReadinessLockoutCondition(
                ...
                description="The watch is outside the ES allowed-hours session window.",
                provenance=[
                    _field_provenance(
    ...
    conditions.append(
        ReadinessLockoutCondition(
            ...
            kind="SESSION_LOCKOUT",
            description="The watch hard-stops at the ES allowed-hours close.",
            provenance=[
                _field_provenance(
...
def _build_price_touch_trigger(
    ...
    operator_claims = [
        NarrativeClaim(
            ...
            category="trigger",
            statement=(
                f"Escalate for fresh Stage A+B reanalysis if ES tags {_format_price(level_price)} "
...
def _coerce_es_watch(watch: ReadinessWatchV1 | Mapping[str, Any]) -> ReadinessWatchV1:
    validated = watch if isinstance(watch, ReadinessWatchV1) else ReadinessWatchV1.model_validate(watch)
    if validated.contract != "ES":
        raise ValueError("Readiness v2 ES slice only supports ES watches.")
    return validated
...
def _build_watch_state_claim(
    ...
    *,
    ...
    if state == "READY_FOR_REANALYSIS":
        statement = (
            f"ES watch is ready_for_reanalysis after {active_trigger_family} observed at "
        ...
        statement = (
            f"ES watch is context_contaminated because packet freshness is {context_snapshot.freshness_state.lower()} "
            f"at age {context_snapshot.packet_age_seconds} seconds."
        )
    elif state == "LOCKED_OUT":
        statement = (
            f"ES watch is locked_out because deterministic lockout reasons are "
            f"{', '.join(context_snapshot.lockout_reasons)}."
        )
    elif state == "THESIS_INVALIDATED":
        statement = (
            f"ES watch is thesis_invalidated because current price {_format_price(packet.market_packet.current_price)} "
            f"breached the active invalidation level."
        )
    elif state == "TERMINAL_EXPIRED":
        statement = (
            f"ES watch is terminal_expired because evaluation time {_render_datetime(evaluation_time)} "
            f"reached the watch expiry."
        )
    elif active_trigger_family == "price_level_touch":
        statement = (
            f"ES watch is {state.lower()} with price-touch monitoring at {_format_price(price_level or 0.0)} "
        ...
        statement = (
            f"ES watch is {state.lower()} with a timed recheck at {_render_datetime(recheck_at_time or evaluation_time)} "
    ...
    *,
    ...
    _append_change(
        ...
        from_value=(
            ...
            f"{prior_watch.context_snapshot.breadth}|"
            f"{prior_watch.context_snapshot.index_cash_tone}"
        ),
        to_value=(
            f"{current_context.macro_release_context}|"
            f"{current_context.breadth}|"
            f"{current_context.index_cash_tone}"
        ),
        detail="Macro release or internals posture changed on the validated packet.",
    )
    _append_change(
        changes,
...
def _structure_claim_text(
    ...
    if feature_snapshot.directional_lean == "BEARISH":
        return (
            f"ES is trading below VWAP {_format_price(market.vwap)} with negative directional posture "
            f"toward developing VAL at {_format_price(market.current_session_val)}."
        )
    if feature_snapshot.directional_lean == "TWO_WAY":
        return (
            f"ES is rotating inside developing value between {_format_price(market.current_session_val)} "
            f"and {_format_price(market.current_session_vah)} with no clean directional edge yet."
        )
    return (
        f"ES is holding above VWAP {_format_price(market.vwap)} and session POC "
...
def _coerce_es_packet(packet: HistoricalPacket | Mapping[str, Any]) -> HistoricalPacket:
    validated = packet if isinstance(packet, HistoricalPacket) else validate_historical_packet(packet)
    if validated.market_packet.contract != "ES":
        raise ValueError("Readiness v2 ES slice only supports ES packets.")
    if not isinstance(validated.contract_specific_extension, ESContractSpecificExtension):
        raise ValueError("Readiness v2 ES slice requires an ES contract extension.")
    return validated
...
def _artifact_id(kind: str, evaluation_time: datetime) -> str:
    return f"es-{kind}-{evaluation_time.strftime('%Y%m%dT%H%M%SZ').lower()}"
...
def _market_internals_context(extension: ESContractSpecificExtension) -> str:
    return f"Breadth is {extension.breadth} and index cash tone is {extension.index_cash_tone}."
...
def _validate_watch_triggers(
    ...
    if not triggers:
        raise ValueError("ES readiness watch requires at least one query trigger.")

    seen_families: set[str] = set()
    for trigger in triggers:
        if trigger.contract != "ES":
            raise ValueError("Readiness v2 ES slice only supports ES query triggers.")
...
def _governance_lock_reasons(packet: HistoricalPacket) -> list[str]:
    reasons: list[str] = []
    trades_today = packet.challenge_state.trades_today_by_contract.model_dump(by_alias=True)["ES"]
    ...
    if any(position.contract == "ES" for position in packet.challenge_state.current_open_positions):
        reasons.append("existing_es_position_open")
    return reasons
