...
def build_cl_premarket_briefing(
    packet: HistoricalPacket | Mapping[str, Any],
    *,
    created_at: datetime | None = None,
) -> PremarketBriefingV1:
    validated = _coerce_cl_packet(packet)
...
def build_cl_initial_query_triggers(
    packet: HistoricalPacket | Mapping[str, Any],
    briefing: PremarketBriefingV1,
) -> list[QueryTriggerV1]:
    validated = _coerce_cl_packet(packet)
    if briefing.contract != "CL":
        raise ValueError("CL readiness v2 only accepts CL briefings.")
...
def build_cl_initial_readiness_watch(
    packet: HistoricalPacket | Mapping[str, Any],
    briefing: PremarketBriefingV1,
...
def update_cl_readiness_watch(
    prior_watch: ReadinessWatchV1 | Mapping[str, Any],
    packet: HistoricalPacket | Mapping[str, Any],
    *,
    evaluation_timestamp: datetime,
) -> ReadinessWatchV1:
    prior = _coerce_cl_watch(prior_watch)
    validated = _coerce_cl_packet(packet)
...
def replay_cl_readiness_sequence(
    packet: HistoricalPacket | Mapping[str, Any],
    ...
    *,
    ...
    validated = _coerce_cl_packet(packet)
    briefing = build_cl_premarket_briefing(validated, created_at=bootstrap_evaluation_time)
    query_triggers = build_cl_initial_query_triggers(validated, briefing)
    watch = build_cl_initial_readiness_watch(
        validated,
        briefing,
        query_triggers,
    ...
    for step_index, update in enumerate(updates, start=1):
        ...
        watch = update_cl_readiness_watch(
            watch,
            update_packet,
...
def _coerce_replay_update(
    ...
    return shared_coerce_replay_update(update, packet_coercer=_coerce_cl_packet)
...
def _build_watch_context_snapshot(
    ...
    if not isinstance(extension, CLContractSpecificExtension):
        raise ValueError("CL readiness v2 requires a CL contract extension.")
    ...
    return CLReadinessWatchContextSnapshot(
        evaluation_timestamp=evaluation_time,
        ...
        eia_timing_context=_eia_timing_context(extension),
        realized_volatility_context=extension.realized_volatility_context,
        liquidity_sweep_summary=_liquidity_sweep_summary(extension),
        dom_liquidity_summary=_dom_liquidity_summary(extension),
...
def _build_feature_snapshot(
    ...
    return CLPremarketBriefingFeatureSnapshot(
        current_price=packet.market_packet.current_price,
...
def _build_thesis_claims(
    ...
    feature_snapshot: CLPremarketBriefingFeatureSnapshot,
    evaluation_time: datetime,
) -> list[NarrativeClaim]:
    extension = packet.contract_specific_extension
    if not isinstance(extension, CLContractSpecificExtension):
    ...
    event_statement = (
        f"The next scheduled event is {next_event.name} at {next_event.time.isoformat()}, which can hard-stop the CL watch as the lockout window approaches."
    ...
    claims = [
        NarrativeClaim(
            ...
            provenance=[
                _field_provenance(
                    ...
                    "Current CL price anchors the value-context portion of the thesis.",
                ...
                _field_provenance(
                    ...
                    "Current session VAH anchors the CL upside value boundary.",
                    observed_at=_as_utc(packet.market_packet.timestamp),
                ),
                _field_provenance(
                    "current_session_val",
                    ...
                    "Current session VAL anchors the CL downside value boundary.",
                    observed_at=_as_utc(packet.market_packet.timestamp),
                ),
                _derived_provenance(
                    "directional_lean",
        ...
        NarrativeClaim(
            ...
            category="flow",
            statement=(
                f"{_liquidity_sweep_summary(extension)} {_dom_liquidity_summary(extension)} "
            ...
            provenance=[
                _field_provenance(
                    ...
                    "contract_specific_extension.liquidity_sweep_summary",
                    _liquidity_sweep_summary(extension),
                    "Liquidity sweep summary is carried directly into the CL flow alignment claim.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "dom_liquidity_summary",
                    "contract_specific_extension.dom_liquidity_summary",
                    _dom_liquidity_summary(extension),
                    "DOM liquidity summary is carried directly into the CL flow alignment claim.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "cumulative_delta",
        ...
        NarrativeClaim(
            ...
            category="risk",
            statement=(
                f"EIA timing is '{feature_snapshot.eia_timing_context}', realized volatility is "
                f"'{feature_snapshot.realized_volatility_context}', and cross-market context is "
                f"'{feature_snapshot.cross_market_context}'."
            ),
            evidence_fields=[
            ...
            provenance=[
                _derived_provenance(
                    "eia_timing_context",
                    "derived.eia_timing_context",
                    feature_snapshot.eia_timing_context,
                    "EIA timing context is deterministically derived from the CL contract extension.",
                    observed_at=evaluation_time,
                ),
                _field_provenance(
                    "realized_volatility_context",
                    "contract_specific_extension.realized_volatility_context",
                    feature_snapshot.realized_volatility_context,
                    "Realized volatility context is taken directly from the CL contract extension.",
                    observed_at=evaluation_time,
                ),
                _derived_provenance(
                    "cross_market_context",
                    "derived.cross_market_context",
                    feature_snapshot.cross_market_context,
                    "Cross-market context is deterministically rendered from the validated CL packet context map.",
                    observed_at=evaluation_time,
                ),
            ],
        ),
    ...
    if price_trigger is not None:
        summaries.append(
            BriefingTriggerSummary(
                ...
                summary=(
                    f"Escalate only if CL tags {_format_price(price_trigger[1])}, "
...
def _build_lockout_conditions(
    ...
    if _session_access_state(packet, evaluation_time) == "OUTSIDE_ALLOWED_HOURS":
        conditions.append(
            ReadinessLockoutCondition(
                ...
                description="The watch is outside the CL allowed-hours session window.",
                provenance=[
                    _field_provenance(
    ...
    conditions.append(
        ReadinessLockoutCondition(
            ...
            kind="SESSION_LOCKOUT",
            description="The watch hard-stops at the CL allowed-hours close.",
            provenance=[
                _field_provenance(
...
def _build_price_touch_trigger(
    ...
    operator_claims = [
        NarrativeClaim(
            ...
            category="trigger",
            statement=(
                f"Escalate for fresh Stage A+B reanalysis if CL tags {_format_price(level_price)} "
...
def _coerce_cl_watch(watch: ReadinessWatchV1 | Mapping[str, Any]) -> ReadinessWatchV1:
    validated = watch if isinstance(watch, ReadinessWatchV1) else ReadinessWatchV1.model_validate(watch)
    if validated.contract != "CL":
        raise ValueError("Readiness v2 CL slice only supports CL watches.")
    return validated
...
def _build_watch_state_claim(
    ...
    *,
    ...
    if state == "READY_FOR_REANALYSIS":
        statement = (
            f"CL watch is ready_for_reanalysis after {active_trigger_family} observed at "
        ...
        statement = (
            f"CL watch is context_contaminated because packet freshness is {context_snapshot.freshness_state.lower()} "
            f"at age {context_snapshot.packet_age_seconds} seconds."
        )
    elif state == "LOCKED_OUT":
        statement = (
            f"CL watch is locked_out because deterministic lockout reasons are "
            f"{', '.join(context_snapshot.lockout_reasons)}."
        )
    elif state == "THESIS_INVALIDATED":
        statement = (
            f"CL watch is thesis_invalidated because current price {_format_price(packet.market_packet.current_price)} "
            f"breached the active invalidation level."
        )
    elif state == "TERMINAL_EXPIRED":
        statement = (
            f"CL watch is terminal_expired because evaluation time {_render_datetime(evaluation_time)} "
            f"reached the watch expiry."
        )
    elif active_trigger_family == "price_level_touch":
        statement = (
            f"CL watch is {state.lower()} with price-touch monitoring at {_format_price(price_level or 0.0)} "
        ...
        statement = (
            f"CL watch is {state.lower()} with a timed recheck at {_render_datetime(recheck_at_time or evaluation_time)} "
...
def _detect_watch_changes(
    ...
    *,
    ...
    _append_change(
        ...
        from_value=(
            f"{prior_watch.context_snapshot.eia_timing_context}|"
            f"{prior_watch.context_snapshot.realized_volatility_context}|"
            f"{prior_watch.context_snapshot.liquidity_sweep_summary}|"
            f"{prior_watch.context_snapshot.dom_liquidity_summary}|"
            f"{prior_watch.context_snapshot.cross_market_context}"
        ),
        to_value=(
            f"{current_context.eia_timing_context}|"
            f"{current_context.realized_volatility_context}|"
            f"{current_context.liquidity_sweep_summary}|"
            f"{current_context.dom_liquidity_summary}|"
            f"{current_context.cross_market_context}"
        ),
        detail="EIA timing, realized volatility, liquidity posture, or cross-market context changed on the validated packet.",
    )
    _append_change(
        changes,
...
def _structure_claim_text(
    ...
    if feature_snapshot.directional_lean == "BEARISH":
        return (
            f"CL is trading below VWAP {_format_price(market.vwap)} with negative directional posture "
            f"toward developing VAL at {_format_price(market.current_session_val)}."
        )
    if feature_snapshot.directional_lean == "TWO_WAY":
        return (
            f"CL is rotating inside developing value between {_format_price(market.current_session_val)} "
            f"and {_format_price(market.current_session_vah)} with no clean directional edge yet."
        )
    return (
        f"CL is holding above VWAP {_format_price(market.vwap)} and session POC "
...
def _coerce_cl_packet(packet: HistoricalPacket | Mapping[str, Any]) -> HistoricalPacket:
    validated = packet if isinstance(packet, HistoricalPacket) else validate_historical_packet(packet)
    if validated.market_packet.contract != "CL":
        raise ValueError("Readiness v2 CL slice only supports CL packets.")
    if not isinstance(validated.contract_specific_extension, CLContractSpecificExtension):
        raise ValueError("Readiness v2 CL slice requires a CL contract extension.")
    return validated
...
def _artifact_id(kind: str, evaluation_time: datetime) -> str:
    return f"cl-{kind}-{evaluation_time.strftime('%Y%m%dT%H%M%SZ').lower()}"
...
def _cross_market_context(packet: HistoricalPacket) -> str:
    ...
    fragments: list[str] = []
    for key in ("equity_tone", "dxy_direction", "bond_yield_direction"):
...
def _eia_timing_context(extension: CLContractSpecificExtension) -> str:
    timing = extension.eia_timing
    if timing.status == "not_today":
        return "No EIA release is scheduled for today."
    if timing.status == "released":
        return (
            f"EIA Petroleum Status Report released {timing.minutes_since} minutes ago at "
            f"{_render_datetime(timing.scheduled_time)}."
        )
    return (
        f"EIA Petroleum Status Report is scheduled for "
        f"{_render_datetime(timing.scheduled_time)} in "
        f"{timing.minutes_until} minutes."
    )


def _liquidity_sweep_summary(extension: CLContractSpecificExtension) -> str:
    return extension.liquidity_sweep_summary or "Liquidity sweep posture is unavailable."


def _dom_liquidity_summary(extension: CLContractSpecificExtension) -> str:
    return extension.dom_liquidity_summary or "DOM liquidity posture is unavailable."
...
def _validate_watch_triggers(
    ...
    if not triggers:
        raise ValueError("CL readiness watch requires at least one query trigger.")

    seen_families: set[str] = set()
    for trigger in triggers:
        if trigger.contract != "CL":
            raise ValueError("Readiness v2 CL slice only supports CL query triggers.")
...
def _governance_lock_reasons(packet: HistoricalPacket) -> list[str]:
    reasons: list[str] = []
    trades_today = packet.challenge_state.trades_today_by_contract.model_dump(by_alias=True)["CL"]
    ...
    if any(position.contract == "CL" for position in packet.challenge_state.current_open_positions):
        reasons.append("existing_cl_position_open")
    return reasons
